...
class DriftDetector:
    ...
    def detect_feature_drift(self, baseline_data, current_data):
        ...
        for feature in baseline_data.columns:
            ...
            psi_test = self.population_stability_index(
            ...
            feature_drift = {
                "feature": feature,
                "tests": {"ks_test": ks_test, "psi": psi_test},
                "drift_detected": ks_test["drift_detected"]
                or psi_test["drift_detected"],
            }
        ...
        logger.info(
            f"Drift detection complete. {drift_report['summary']['features_with_drift']} features with drift"
        )
    ...
    def detect_model_performance_drift(self, baseline_metrics, current_metrics):
        ...
        for metric_name in baseline_metrics.keys():
            ...
            if status == "degraded":
                logger.warning(
                    f"Performance degradation detected in {metric_name}: {baseline_value:.4f} -> {current_value:.4f}"
                )
    ...
    def should_trigger_retraining(self, drift_report, performance_report=None):
        ...
        if drift_report["summary"]["drift_percentage"] > 20:
            reasons.append(
                f"Significant feature drift detected ({drift_report['summary']['drift_percentage']:.1f}%)"
            )
        ...
        if performance_report:
            if len(performance_report["summary"]["degraded_metrics"]) > 0:
                reasons.append(
                    f"Performance degradation in {len(performance_report['summary']['degraded_metrics'])} metrics"
                )
...
def main():
    ...
    perf_report = detector.detect_model_performance_drift(
