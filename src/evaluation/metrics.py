...
def business_metric_cost(y_true, y_pred, fp_cost=100, fn_cost=500):
    ...
    logger.info(f"Business Cost - FP: {fp} (${fp*fp_cost}), FN: {fn} (${fn*fn_cost}), Total: ${total_cost}")
...
def calculate_optimal_threshold(y_true, y_pred_proba, metric='youden'):
    ...
    for threshold in thresholds:
        ...
        if metric == 'youden':
            ...
            score = -business_metric_cost(y_true, y_pred)  # Negative because we minimize cost
        else:
    ...
    logger.info(f"Optimal threshold ({metric}): {best_threshold:.3f} with score: {best_score:.4f}")
...
class MetricTracker:
    ...
    def detect_degradation(self, metric_name, threshold=0.05):
        ...
        if degradation > threshold:
            logger.warning(f"Metric degradation detected for {metric_name}: {degradation:.4f}")
            return True
