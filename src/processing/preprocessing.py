...
class DataPreprocessor:
    ...
    def prepare_data(
        self, df, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, random_state=42
    ):
    ...
    def save_processed_data(self, data_dict, output_path):
        ...
        val_data.to_csv(
            os.path.join(output_path, "validation", "validation.csv"),
            index=False,
            header=False,
        )
        ...
        test_data.to_csv(
            os.path.join(output_path, "test", "test.csv"), index=False, header=False
        )
        ...
        with open(
            os.path.join(output_path, "model", "feature_metadata.json"), "w"
        ) as f:
        ...
        with open(
            os.path.join(output_path, "reports", "data_quality_report.json"), "w"
        ) as f:
