...
def check_model_registry(model_package_group_name="diabetes-classifier-model-group", region="us-east-1"):
    ...
    try:
        ...
        if not response['ModelPackageSummaryList']:
            ...
            print("  2. Models didn't meet quality thresholds (check evaluation metrics)")
        ...
        if approved_count > 0:
            ...
            print("  python src/deployment/deploy.py --config config/config.yaml --endpoint-name diabetes-classifier\n")
            ...
            for pkg in response['ModelPackageSummaryList']:
                if pkg.get('ModelApprovalStatus') == 'PendingManualApproval':
                    print(f"  aws sagemaker update-model-package --model-package-arn {pkg['ModelPackageArn']} --model-approval-status Approved")
                    break
        else:
