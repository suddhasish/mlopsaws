...
class DiabetesPipeline:
    ...
    def __init__(self, config_path='config/config.yaml', environment='dev'):
        ...
        """
        logger.info(f"Initializing Diabetes Classification Pipeline for environment: {environment}")
            ...
            with open(env_config_path, 'r') as f:
                ...
                for env_config in env_configs:
                    if environment in env_config:
                        ...
                        logger.info(f"Loaded environment-specific configuration for {environment}")
                        break
    ...
    def create_pipeline_parameters(self):
        ...
        parameters = {
            ...
            'model_approval_status': ParameterString(
                ...
                default_value=self.config['sagemaker']['model_registry']['approval_status']
            ),
            ...
            'eta': ParameterFloat(
                name='Eta',
                default_value=self.config['model']['hyperparameters']['eta']
            ),
    ...
    def create_model_registration_step(self, step_train, step_eval, evaluation_report, parameters):
        ...
        step_register = RegisterModel(
            ...
            model_package_group_name=self.config['sagemaker']['model_registry']['model_package_group_name'],
    ...
    def create_condition_step(self, step_eval, evaluation_report, step_register):
        ...
        logger.info(f"  - Min Accuracy: {self.config['evaluation']['approval_thresholds']['min_accuracy']}")
        logger.info(f"  - Min F1 Score: {self.config['evaluation']['approval_thresholds']['min_f1_score']}")
        logger.info(f"  - Min ROC-AUC: {self.config['evaluation']['approval_thresholds']['min_roc_auc']}")
    ...
    def execute_pipeline(self, pipeline):
        ...
        logger.info(f"Approval status: {self.config['sagemaker']['model_registry']['approval_status']}")
...
def main():
    ...
    parser.add_argument('--execute', action='store_true',
                      help='Execute the pipeline after creation')
    
    args = parser.parse_args()
    
    # Create pipeline with environment
    pipeline_builder = DiabetesPipeline(config_path=args.config, environment=args.environment)
