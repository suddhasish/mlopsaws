...
# Attach EventBridge permissions (for scheduled events and auto-shutdown)
aws iam attach-role-policy `
  --role-name GitHubActions-MLOps-Dev `
  --policy-arn arn:aws:iam::aws:policy/AmazonEventBridgeFullAccess `
  --profile mlops-dev

# Attach Lambda permissions (for auto-shutdown functions)
aws iam attach-role-policy `
  --role-name GitHubActions-MLOps-Dev `
...
- **EventBridge** - Create scheduled rules for auto-shutdown and monitoring
- **Lambda** - Deploy auto-shutdown functions and event handlers
...
# Check attached policies (should show 8 policies)
...
### Issue 6: Terraform Format Check Failures
...
**Root Cause:** Inconsistent spacing before inline comments in `.tfvars` files. Terraform requires EXACTLY 2 spaces before `#` in inline comments.

**Solution:**

```powershell
# Incorrect formatting (will fail):
enable_auto_shutdown = true    # 4 spaces - WRONG
budget_amount = 100 # 1 space - WRONG
aws_region = "us-east-1"   # 3 spaces - WRONG

# Correct formatting (will pass):
enable_auto_shutdown = true  # Exactly 2 spaces - CORRECT
budget_amount = 100  # Exactly 2 spaces - CORRECT
aws_region = "us-east-1"  # Exactly 2 spaces - CORRECT
```

**Fix all tfvars files:**
...
Edit each file and ensure EXACTLY 2 spaces before every `#` comment:

```hcl
# Before (incorrect):
aws_account_id = "891807086260"    # ← 4 spaces
owner_email = "user@example.com" # ← 1 space

# After (correct):
aws_account_id = "891807086260"  # ← 2 spaces
owner_email = "user@example.com"  # ← 2 spaces
```
...
### Issue 7: GitHub Actions Artifact Deprecation

**Error:**
```
Warning: The following actions uses node12 which is deprecated: actions/upload-artifact@v3
Warning: The following actions uses node12 which is deprecated: actions/download-artifact@v3
```

**Status:** 🔴 **MANUAL FIX**

**Root Cause:** GitHub deprecated artifact actions v3. Need to upgrade to v4.

**Solution:**

Edit `.github/workflows/terraform.yml` and update all artifact actions:
...
**✅ Verification:**
```powershell
# Search for v3 references (should find none after fix)
Select-String -Path ".github/workflows/terraform.yml" -Pattern "artifact@v3"
```

---

### Issue 8: Terraform Directory Structure Error
...
**Root Cause:** GitHub Actions running from wrong directory. Environment folders only contain `terraform.tfvars`, not `.tf` files.

**Directory Structure:**
```
infrastructure/terraform/
├── main.tf              ← Root module (has all .tf files)
├── modules.tf
├── variables.tf
├── outputs.tf
└── environments/
...
Update `.github/workflows/terraform.yml` to run from root directory:

```yaml
# Before (incorrect - runs from environment dir):
- name: Terraform Init
  working-directory: infrastructure/terraform/environments/dev
  run: terraform init
...
# After (correct - run from root with var-file):
- name: Terraform Init
...
**Apply same fix to all environments (dev, staging, production):**
...
# Should generate plan without errors
```

---

### Issue 9: Networking Module Parameter Mismatch

**Error:**
```
Error: Unsupported argument
on modules.tf line 25, in module "networking":
...
**Root Cause:** Networking module expects `tags` parameter, but we're passing `additional_tags`.
...
module "networking" {
  ...
  additional_tags = var.additional_tags  # ← WRONG parameter name
}
...
**Why merge?**
- `tags` expects a flat map of key-value pairs
- We merge default tags with `additional_tags` to ensure required tags are always present
- This pattern allows users to add custom tags via `additional_tags` variable
...
### Issue 10: EC2 Permission Denied

**Error:**
```
Error: error describing EC2 Availability Zones
AccessDenied: User: arn:aws:sts::ACCOUNT:assumed-role/GitHubActions-MLOps-Dev/GitHubActions
is not authorized to perform: ec2:DescribeAvailabilityZones
```

**Status:** 🔴 **MANUAL FIX**

**Root Cause:** IAM role missing EC2 permissions needed to query VPC, subnets, and availability zones.
...
**Why needed?**
- Terraform networking module queries available AZs
- Creates subnets across multiple availability zones
- Describes VPC and security group configurations

**✅ Verification:**
```powershell
# Test EC2 permissions
aws ec2 describe-availability-zones --profile mlops-dev
# Should list availability zones in your region
```

---

### Issue 11: Lambda ZIP Files Missing

**Error:**
```
Error: Error creating Lambda function
InvalidParameterValueException: Could not find file: ./modules/auto_shutdown/shutdown_endpoint.zip
```

**Status:** 🔴 **MANUAL FIX** (Temporary workaround: disable auto-shutdown)

**Root Cause:** Lambda deployment packages (ZIP files) don't exist in the repository.
...
```hcl
# Before:
enable_auto_shutdown = true  # Will fail - ZIP files missing

# After:
enable_auto_shutdown = false  # Disabled until Lambda ZIPs created
```

**Permanent Solution (Create Lambda ZIPs):**

```powershell
# Navigate to auto_shutdown module
cd infrastructure/terraform/modules/auto_shutdown

# Create Lambda deployment packages
# (This would be implemented by creating Python Lambda functions)
...
# Compress-Archive -Path lambda/shutdown_endpoint.py -DestinationPath shutdown_endpoint.zip
# Compress-Archive -Path lambda/startup_endpoint.py -DestinationPath startup_endpoint.zip
```

**Note:** Auto-shutdown feature is optional for dev environments. Disable it to avoid deployment errors until Lambda functions are implemented.
...
cd infrastructure/terraform
terraform validate
# Should succeed with enable_auto_shutdown = false
```

---

### Issue 12: EventBridge/Lambda Permissions Missing

**Error:**
```
Error: Error creating EventBridge rule
AccessDeniedException: User is not authorized to perform: events:TagResource
```

**Status:** 🔴 **MANUAL FIX**

**Root Cause:** IAM role missing EventBridge and Lambda permissions for auto-shutdown module.
...
# Verify (should show 8 policies)
aws iam list-attached-role-policies --role-name GitHubActions-MLOps-Dev --profile mlops-dev
```

**Why needed?**
- **EventBridge** - Create scheduled rules for auto-shutdown
- **Lambda** - Deploy and manage Lambda functions
- Required even if auto-shutdown is disabled (permissions needed for module validation)

**✅ Verification:**
```powershell
# All 8 policies should be attached:
# 1. AmazonSageMakerFullAccess
# 2. AmazonS3FullAccess
# 3. AmazonEC2FullAccess
# 4. IAMFullAccess
...
### Issue 13: SageMaker Model Package Groups Quota Exceeded

**Error:**
```
Error: Error creating SageMaker Model Package Group
ResourceLimitExceeded: The account-level service limit 'Model package groups per account' is 0 Limits
```

**Status:** 🔴 **MANUAL FIX** (Requires AWS quota increase)

**Root Cause:** SageMaker Model Package Groups quota is set to 0 (feature disabled by default in some accounts).

**Check Current Quota:**

```powershell
# List SageMaker quotas
aws service-quotas list-service-quotas `
  --service-code sagemaker `
  --query "Quotas[?contains(QuotaName, 'Model') && contains(QuotaName, 'Package')]" `
  --profile mlops-dev

# Expected output shows quota L-BC8DC54C with Value: 0.0
```

**Solution 1: Request Quota Increase via CLI**

```powershell
# Request increase to 250 (minimum allowed)
...
#     "RequestedQuota": {
#         "Id": "XXXXXXXXX",
#         "QuotaCode": "L-BC8DC54C",
#         "QuotaName": "Maximum number of SageMaker Model Package Groups allowed per account",
#         "DesiredValue": 250.0,
#         "Status": "PENDING"
#     }
# }
...
```powershell
# Check status (use ID from above output)
aws service-quotas get-requested-service-quota-change `
  --request-id XXXXXXXXX `
  --profile mlops-dev

# Possible statuses:
# - PENDING: Waiting for approval (typically 24-48 hours)
# - APPROVED: Quota increased, ready to use
# - DENIED: Request rejected (open support case)
```

**Solution 2: Request via AWS Console (Alternative)**
...
5. Enter: 250 (or your desired value)
6. Click: "Request"
7. Wait: 24-48 hours for approval
```

**Temporary Workaround (Until Quota Approved):**

Comment out SageMaker module in `infrastructure/terraform/modules.tf`:

```hcl
# Temporarily disable until quota approved
# module "sagemaker" {
...
- Large increases (0 → 250): May require AWS support review
- Support ticket: Can expedite (1-2 business days)

**✅ Verification (After Approval):**
```powershell
# Check quota was increased
aws service-quotas get-service-quota `
  --service-code sagemaker `
  --quota-code L-BC8DC54C `
  --profile mlops-dev

# Value should be 250.0 or higher
```
